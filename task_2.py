...
def get_shop_list_for_dishes(tuple_of_dishes, person_count=0):
    for key, value in cook_book.items():
        for ingredient_param in value:
            ingredient_param['quantity'] = int(ingredient_param['quantity']) * person_count
        if key in tuple_of_dishes:
            pprint(value)


get_shop_list_for_dishes(('Запеченный картофель', 'Фахитос'), 23)
